                            ...
                            echo "<div class='card mt-3' style='width: 80rem;' id='postan'>
                                        <div>
                                            ...
                                            <div class='card-body'>
                                                ...
                                                <p>{$rowPost['tags']}</p>
                                                <p style='margin-top:10px;'>Comments ({$comments['Count']})</br>Likes ({$checkLikes['Count']})</p>
                                                <p id='time'>{$date}</p>
                                            </div>
                                        </div>
                                    </div>";
                            ...
                            echo "<div class='card mt-3' style='width: 80rem;' id='postan'>
                                        <div>
                                            <div class='card-body'>
                                                ...
                                                <button style='margin-left:5px;' type='submit' class='btn btn-primary btn-sm'>Comment</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>";
                            ...
                            while($rowComments = mysqli_fetch_assoc($keranjangComments)) { //display all comments
                                                        ...
                                                        echo"
                                                            ...
                                                            if(isset($_SESSION['id'])) { //if logged in{
                                                                ...
                                                                if($_SESSION['role'] == "admin") { //if admin allow to delete reply
                                                                    echo "<p><a style='margin-left:710px;  href='proses_delete.php?id={$rowComments['id']}&type=comment&from={$url}'><button style='font-size:24px;color:white;background-color:red;font-size:15px;'>Delete</button></a></p>";
                                                                } else if(!$checkLikes['Count']) { //check if liked by user
                                                                    echo "<p><a href='proses_like.php?liked_id={$rowComments['id']}&type=comment&action=like&from={$url}'><i class='glyphicon glyphicon-heart' style='color:black;width:30px;margin-right:2px;font-size:25px;'></i></a></p>";
                                                                } else{
                                                                    echo "<p><a href='proses_like.php?liked_id={$rowComments['id']}&type=comment&action=unlike&from={$url}'><i class='glyphicon glyphicon-heart' style='color:red;width:30px;margin-right:2px;font-size:25px;'></i></a></p>";
                                                                }
                                ...
                                echo            "</div>
                                    ...
                                    while($rowReply = mysqli_fetch_assoc($keranjangReply))//display replies
                                    {
                                        ...
                                        echo "<div class='card mt-3' style='width: 75rem; margin-left: -41px;' id='postan'>
                                                    <div>
                                                        <div class='card-body'>";
                                                                echo"
                                                                    <div class='d-flex d-flex-flex-column'>
                                                                        <image style='border-radius:50%; border:2px solid black; height:30px;' src='data:image/png;base64, {$rowUserReply['foto']}' width= 30px/>
                                                                        <a style='margin-left:5px; margin-top:5px; margin-bottom:25px;' href='profile_view.php?id={$rowReply['posted_by']}'>@{$rowUserReply['username']}</a>
                                                                    </div>
                                                                    <p><b>@{$rowUser['username']}</b> {$rowReply['comment']}</p>
                                                                    <p>Likes: {$checkLikes['Count']}</p>
                                                                    <p>{$date}</p>";
                                                                if(isset($_SESSION['id'])) //if logged in
                                                                {
                                                                    ...
                                                                    if($_SESSION['role'] == "admin")//if admin allow to delete reply
                                                                    {
                                                                        echo "<a style='margin-left:660px;  href='proses_delete.php?id={$rowReply['id']}&type=reply&from={$url}'><button style='background-color:red;color:white;'>Delete</button></a><br/>";
                                                                    } else if(!$checkLikes['Count'])//check if liked by user
                                                                    {
                                                                        echo "<a href='proses_like.php?liked_id={$rowReply['id']}&type=comment&action=like&from={$url}'><i class='glyphicon glyphicon-heart' style='color:black;width:30px;margin-right:2px;font-size:25px;'></i></a>";
                                                                    } else
                                                                    {
                                                                        echo "<a href='proses_like.php?liked_id={$rowReply['id']}&type=comment&action=unlike&from={$url}'><i class='glyphicon glyphicon-heart' style='color:red;width:30px;margin-right:2px;font-size:25px;'></i></a>";
                                                                    }
                                                                }
                                        echo            "</div>
                                                    </div>
